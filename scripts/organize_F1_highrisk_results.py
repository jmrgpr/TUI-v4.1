...
def iter_raw_runs():
    """
    Itera sobre los archivos JSON crudos de F1_highrisk que deben organizarse.
    ...
    for path in RAW_DIR.glob("grid*_riskhigh*_seed*_v11.json"):
        # Excluir runs archivados explícitos
...
def parse_grid_from_stem(stem: str) -> str:
    """
    Extrae el tamaño de grid a partir del nombre base del archivo.
    Espera nombres del tipo: grid8_riskhigh_..., grid16_riskhigh_...
    """
...
def write_agent_json(json_data: dict, agent: str, grid: str, base_stem: str) -> Path:
    ...
    # Para dqn_control, añadir hiperparámetros si existen a nivel raíz
...
def organize_f1_results() -> None:
    """
    Organiza F1_highrisk v11 en la estructura científica final:
    un archivo por (grid, agente, seed) en:
        results/v11/F1_highrisk/grid{8,16}/riskhigh/{control,simbiosis,dqn_control}/
    ...
    for json_path in iter_raw_runs():
        ...
        for agent in AGENTS:
            if agent not in json_data:
                # Mantener trazabilidad pero no abortar si falta un agente puntual
