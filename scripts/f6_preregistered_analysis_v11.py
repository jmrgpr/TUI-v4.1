...
F6_DIR = Path("results/v11/F6/F2_redteam/stkH")
F6_ANALYSIS_DIR = Path("results/v11/F6/analysis")
DATA_DIR = Path("results/v11/data")

PSTAR_JSON = F6_ANALYSIS_DIR / "f6_pilot_selection_v11.json"

OUT_PILOT_TABLE = DATA_DIR / "f6_pilot_table_v11.csv"
OUT_PILOT_MD = DATA_DIR / "f6_pilot_selection_v11.md"

OUT_RUNS_CSV = DATA_DIR / "f6_run_metrics_v11.csv"
OUT_STATS_CSV = DATA_DIR / "f6_preregistered_stats_v11.csv"
OUT_MD = DATA_DIR / "f6_preregistered_report_v11.md"
...
def detect_rt_dir() -> Path:
    if PSTAR_JSON.exists():
        selection = json.loads(PSTAR_JSON.read_text(encoding="utf-8"))
        token = str(selection.get("p_star_token") or "").strip()
        if not token:
            raise ValueError(f"p_star_token faltante en {PSTAR_JSON.as_posix()}")
        root = F6_DIR / token
        if not root.exists():
            raise FileNotFoundError(f"Directorio confirmatorio no existe: {root.as_posix()}")
        return root

    candidates = sorted([p for p in F6_DIR.glob("rt*/") if p.is_dir()])
    if len(candidates) == 1:
        return candidates[0]
    raise FileNotFoundError(
        "No se pudo inferir el directorio rt* de F6. Falta p* (PSTAR_JSON) o hay múltiples rt*/ sin selección."
    )
...
def parse_red_team_prob(path: Path) -> float:
    m = re.search(r"_f2rt(\d+)p(\d+)_seed", path.name.lower())
    if not m:
        raise ValueError(f"No se pudo inferir red_team_prob desde filename: {path.name}")
...
def mcnemar_exact_2sided(x: np.ndarray, y: np.ndarray) -> float:
    """
    McNemar exacto (2-sided) sobre pares binarios.
    x,y: 0/1 de igual longitud.
    """
    if len(x) != len(y):
...
def load_runs(rt_root: Path) -> pd.DataFrame:
    rows: list[dict] = []
    ...
    for csv_path in files:
        ...
        rt_prob = parse_red_team_prob(csv_path)
        if budget != BUDGET_B:
    ...
    if missing:
        raise RuntimeError(f"Faltan grupos esperados en F6: {sorted(missing)}")
    return runs
...
def write_pilot_artifacts() -> None:
    """
    Exporta la tabla y reporte del piloto a results/v11/data/ si existe PSTAR_JSON.
    """
    if not PSTAR_JSON.exists():
        return
    selection = json.loads(PSTAR_JSON.read_text(encoding="utf-8"))
    rows = selection.get("pilot", {}).get("rows", [])
    ...
    p_star = selection.get("p_star")
    cfr_star = selection.get("cfr_control_p_star")
    candidates = selection.get("pilot", {}).get("candidates")
    seeds = selection.get("pilot", {}).get("seeds")
    grid = selection.get("pilot", {}).get("grid")
    rule = selection.get("pilot", {}).get("rule")
    ...
    md = [
        ...
        f"- Seeds piloto (no reutilizados): `{seeds}`",
        f"- Grid piloto: `{grid}`",
        f"- Regla: {rule}",
        "",
        f"## Selección",
        "",
        f"- `p*` = **{p_star}**",
        f"- `CFR_control(p*)` = **{cfr_star}**",
        f"- JSON canónico de selección: `{PSTAR_JSON.as_posix()}`",
        "",
        "## Tabla",
        "",
...
def format_report(*, rt_root: Path, runs: pd.DataFrame, stats_rows: list[dict], p_holm: dict[str, float]) -> str:
    group_summary = (
        runs.groupby("group")
        .agg(
            n=("cfr", "size"),
    ...
    lines = [
        ...
        "",
        f"Endpoint primario: **CFR** (Catastrophic Failure Rate) = proporción de runs con `catastrophes_total >= B` (budget-exhaustion). Budget congelado: `B={BUDGET_B}`.",
        "",
    ...
    lines.extend(
        [
            ...
            f"- Piloto p*: `{OUT_PILOT_MD.as_posix()}` y `{OUT_PILOT_TABLE.as_posix()}`",
            "",
        ]
    )
    return "\n".join(lines)
...
def main() -> None:
    ...
    OUT_MD.write_text(format_report(rt_root=rt_root, runs=runs, stats_rows=stats_rows, p_holm=p_holm), encoding="utf-8")
