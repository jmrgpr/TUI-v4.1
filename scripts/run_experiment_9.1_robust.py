"""
Script de Ejecución - Experimento 9.1: Validación Estadística Robusta (N=10) del Curriculum Learning
====================================================================================================

Experimento preregistrado para validar Hipótesis H9.1.1-H9.4.1 con N=10 seeds:
- H9.1.1: Curriculum > DirectoS1 (ratio_final ≥ 0.70 con IC que NO cruza threshold)
- H9.2.1: Curriculum ≈ Control (p<0.05, Cohen's d>0.5, adecuadamente powered)
- H9.3.1: Prudencia mantenida (tripwires curriculum ≤ control)
- H9.4.1: Degradación gradual (0/10 seeds colapso súbito)

Diseño: 3 grupos × 10 seeds = 30 configuraciones
        - CURRICULUM: 4 etapas (s=0.0→0.25→0.5→1.0), 75 eps/etapa, transfer learning
        - DIRECTO_S1: 300 eps con s=1.0 constante (control positivo)
        - CONTROL_S0: 300 eps con s=0.0 sin shaping (baseline capacidad)

DIFERENCIAS vs v9:
    - N=10 seeds (vs N=3) → Potencia 60-80% (vs 18%)
    - Seeds: [42, 123, 456, 789, 101112, 131415, 161718, 192021, 222324, 252627]
    - Checkpoints cada 10 configs (3h aprox)
    - Output: results/pgf_v9.1/resultados/

Uso:
    # Ejecución completa v9.1 (30 configs, ~6 horas)
    python scripts/run_experiment_9.1_robust.py
    
    # Test mode (2 seeds × 30 eps, ~1 min)
    python scripts/run_experiment_9.1_robust.py --test_mode
    
    # Solo curriculum (debugging)
    python scripts/run_experiment_9.1_robust.py --groups Curriculum
    
    # Seed específica
    python scripts/run_experiment_9.1_robust.py --seeds 42 123

Autor: TUI v4.1 Research Team
Fecha: 4 diciembre 2025
Preregistro: results/pgf_v9.1/PREREGISTRO_v9.1.md v1.0 (commit 089b258)
Predecesor: v9 (N=3, commit 8c19692)
"""
...
# Seeds (N=10 para validación robusta vs N=3 v9 original)
SEEDS = [42, 123, 456, 789, 101112, 131415, 161718, 192021, 222324, 252627]
...
CHECKPOINT_FREQ = 10  # Checkpoint cada 10 configs (3h aprox)
...
Ejemplos:
  ...
  python scripts/run_experiment_9_curriculum.py --seeds 42
    ...
    if args.test_mode:
        groups = ['Curriculum', 'DirectoS1', 'ControlS0']
        seeds = [42, 789]  # 2 seeds para test (6 configs)
        episodes = TEST_MODE_EPISODES
        print("\n🧪 MODO TEST: 6 configs (2 seeds × 3 grupos) × 30 episodios (~1 min)")
    else:
        groups = args.groups or GROUPS
        seeds = args.seeds or SEEDS
        episodes = args.episodes or DEFAULT_EPISODES
        print("\n📋 MODO VALIDACIÓN: 30 configs (10 seeds × 3 grupos) × 300 episodios (~6 horas)")
    
    grid_size = args.grid_size
    
    # Output directory (v9.1 específico)
    output_dir = Path('results/pgf_v9.1/resultados')
    ...
    print(f"\n📊 EXPERIMENTO v9.1: {len(configs)} configuraciones")
    ...
    if args.test_mode:
        print(f"   Tiempo estimado: ~1 minuto")
    else:
        print(f"   Tiempo estimado: ~{len(configs) * 12:.0f} minutos (~{len(configs) * 12 / 60:.1f} horas)")
        print(f"   Checkpoints: cada {CHECKPOINT_FREQ} configs (~3h)")
    ...
    for i, config in enumerate(configs, 1):
        ...
        # Checkpoint cada 10 configs (v9.1)
        if i % CHECKPOINT_FREQ == 0 and i < len(configs):
            elapsed = (time.time() - start_time) / 60
            remaining = (len(configs) - i) * (elapsed / i)
            print(f"\n⏱️  CHECKPOINT {i//CHECKPOINT_FREQ}: {i}/{len(configs)} completo ({elapsed:.1f} min, ~{remaining:.1f} min restantes)")
            print(f"      Progreso: {i/len(configs)*100:.1f}% | Seeds completadas: {len(set([all_metrics[j]['config']['seed'] for j in range(i)]))}")
    
    # Resumen final
    total_time = (time.time() - start_time) / 60
    print(f"\n\n{'='*70}")
    print(f"✅ EXPERIMENTO v9.1 COMPLETADO (N=10 VALIDACIÓN)")
    print(f"{'='*70}")
