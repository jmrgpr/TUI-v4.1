...
F5_DIR = Path("results/v11/F5")
RAW_BASE = F5_DIR / "raw" / "F2_redteam"
...
def parse_stem(stem: str) -> dict:
    # grid16_riskhigh_r1p2_f2rt0p1_seed123_stkH_b3_m0p2_v11
    m = re.match(
        r"^grid(?P<grid>\d+)_riskhigh_(?P<risk>r\d+p\d+)_(?P<cond>f2rt0p1)_seed(?P<seed>\d+)_(?P<stakes>stkH)_b(?P<budget>\d+)_m(?P<mix_a>\d+)p(?P<mix_b>\d+)_v11$",
        stem,
    )
    if not m:
...
def agent_out_dir(condition: str, stakes_token: str, grid: int, agent: str) -> Path:
    return F5_DIR / condition / stakes_token / f"grid{grid}" / "riskhigh" / agent


def organize_f5_results() -> None:
    """
    Organiza F5 en estructura final:
    - results/v11/F5/F2_redteam/stkH/grid{8,16}/riskhigh/{control,simbiosis}/
    ...
    Nota: `sim.prototipo_rl_simbiosis` genera `control`+`simbiosis` en un mismo run crudo.
    """
