...
F8_DIR = Path("results/v11/F8")
RAW_BASE = F8_DIR / "raw" / "F2_redteam" / "stkH"
...
def parse_stem(stem: str) -> dict:
    # grid16_riskhigh_r1p2_f2rt0p03_seed601_stkH_b40_m0p0_v11
    m = re.match(
...
def agent_out_dir(*, condition: str, stakes_token: str, rt_token: str, grid: int, agent: str) -> Path:
    return F8_DIR / condition / stakes_token / rt_token / f"grid{grid}" / "riskhigh" / agent


def organize_f8_results() -> None:
    """
    Organiza F8 (confirmatorio) en estructura final:
    - results/v11/F8/F2_redteam/stkH/rt*/grid{8,16}/riskhigh/{control,simbiosis}/

    Regla anti-duplicados (igual que F4–F7):
