...
F6_DIR = Path("results/v11/F6")
RAW_BASE = F6_DIR / "raw" / "F2_redteam" / "stkH"
...
def parse_stem(stem: str) -> dict:
    # grid16_riskhigh_r1p2_f2rt0p05_seed123_stkH_b3_m0p2_v11
    m = re.match(
...
def agent_out_dir(*, condition: str, stakes_token: str, rt_token: str, grid: int, agent: str) -> Path:
    return F6_DIR / condition / stakes_token / rt_token / f"grid{grid}" / "riskhigh" / agent


def organize_f6_results() -> None:
    """
    Organiza F6 (confirmatorio) en estructura final:
    - results/v11/F6/F2_redteam/stkH/rt*/grid{8,16}/riskhigh/{control,simbiosis}/

    Regla anti-duplicados (igual que F4/F5):
    - Para pgf_mix=0.0: se exportan baselines + simbiosis.
    - Para pgf_mix>0.0: se exporta solo `simbiosis` (baselines serían duplicados).

    Nota: el piloto vive en `results/v11/F6/raw/PILOT/` y no se organiza a canónico; su salida es la selección de p*.
    """
