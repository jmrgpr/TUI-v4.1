...
class Agent(Event):
    ...
    P_genuino: float = 0.0  # Propósito genuino / Genuine purpose
    eta_extendido: float = 0.0  # Eficiencia extendida / Extended efficiency
    PGF: float = 0.0  # Principio de Gradiente de Fracaso / Failure Gradient Principle
    # Componentes propósito genuino / Genuine purpose components
    C_costo: float = 0.0  # Costo / Cost
    S_auto: float = 0.0  # Autonomía / Autonomy
    R_robust: float = 0.0  # Robustez / Robustness
    I_rep: float = 0.0  # Reputación / Reputation
    # Acciones posibles / Possible actions
    ...
    def calcular_metricas(self, env, info, step):
        ...
        # Autonomía / Autonomy
        self.S_auto = 1.0 if self.purpose != "survive_and_help" else 0.7
        # Robustez / Robustness
        ...
        A_t = self.alignment * self.P_genuino
        delta_C_t = abs(env.resources - self.resources)
        ...
        self.eta_extendido = (delta_I_useful * self.alignment) / (C_total_norm + beta * self.P_riesgo)
