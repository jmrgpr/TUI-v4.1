...
## Nota importante sobre reproducibilidad de reward_env_total

El presente manifiesto solo incluye los hashes de los archivos CSV canónicos (`*_episodes.csv`). Sin embargo, la métrica `reward_env_total` se calcula a partir de los archivos JSON generados por cada run (campo `reward_env_evol` en los JSON de salida). Para una reproducción completa y verificable de los resultados, se requiere acceso a estos archivos JSON fuente.
