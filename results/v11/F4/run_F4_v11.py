...
# Preregistro F4: stakes = penalización adicional solo si hay starvation.
STAKES = {
    "stkL": 0.0,
    "stkH": -300.0,
}
...
def iter_jobs(seeds: list[int], grids: list[int], pgf_mix_values: list[float], episodes: int) -> list[dict]:
    jobs = []
    for stakes_token, penalty in STAKES.items():
        for grid in grids:
            for seed in seeds:
                for mix in pgf_mix_values:
                    ...
                    stem = f"grid{grid}_riskhigh_r1p2_f2rt0p1_seed{seed}_{stakes_token}_{mix_token}_v11"
                    ...
                    jobs.append(
                        {
                            ...
                            "catastrophe_penalty": float(penalty),
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Runner F4 (v11): F2_redteam fijo, stakes {stkL,stkH} + pgf_mix {0.0,0.2}."
    )
    ...
    for idx, job in enumerate(jobs, start=1):
        cmd = build_cmd(
            ...
            catastrophe_penalty=job["catastrophe_penalty"],
            output_prefix=job["output_prefix"],
        )
        print(
            f"\n[{idx}/{len(jobs)}] F2_redteam grid={job['grid']} seed={job['seed']} "
            f"stakes={job['stakes_token']} catastrophe_penalty={job['catastrophe_penalty']} pgf_mix={job['pgf_mix']}"
        )
